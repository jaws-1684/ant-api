...
describe("/api/chats", () => {
  ...
  describe("unauthenticated requests", () => {
    ...
    it("should reject deleting a chat", async () => {
      ...
      await api.delete("/api/chats/" + id).expect(401);
    });
  ...
  describe("authenticated requests", () => {
    it("returns chats as json", async () => {
      const { chatList, accessToken } = await resolve([
        "chatList",
        "accessToken",
      ]);
    ...
    it("should fetch messages for a chat", async () => {
      const { chat, accessToken, messageList } = await resolve([
        "chat",
        "accessToken",
        "messageList",
      ]);
    ...
    it("should create a chat", async () => {
      const { chatPayload, accessToken, participants } = await resolve([
        "chatPayload",
        "accessToken",
        "participants",
      ]);
      const response = await agent
        ...
        .expect(201)
        .expect("Content-Type", /application\/json/);
      expect(response.body.participants.map((p: UserDTO) => p.id)).toEqual(
    ...
    it("should not delete a chat of another user", async () => {
      ...
      const chat = await chatService.addChat({
        participants: [friend.id, randomUser.id],
      });
