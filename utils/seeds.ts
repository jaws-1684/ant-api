...
try {
    const uEntries = await Promise.all(userEntries.map(async u => ({...u,
    ...
    const senderMessages = messageEntries.slice(0, messageEntries.length / 2).map((content => {
        return {
            userId: senderId,
            chatId,
            content,
    ...
    const receiverMessages = messageEntries.slice(messageEntries.length / 2).map((content => {
        return {
            userId: receiverId,
            chatId,
            content,
