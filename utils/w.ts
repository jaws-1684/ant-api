...
type MergeSchemas<Schemas extends WObject<any, any>[]> = Schemas extends [
  infer Head,
  ...infer Tail,
]
...
const isObject = (object: unknown): object is Record<any, any> =>
...
const isObjectId = (id: unknown): id is string =>
  isString(id) && OBJECT_ID_REGEX.test(id);
// -------------------------------------------------------------------------------------
// Exports
// -------------------------------------------------------------------------------------
...
class WOptional<T extends WType<any>> extends WType<undefined | Infer<T>> {
  #schema: T;
...
class WNullable<T extends WType<any>> extends WType<null | Infer<T>> {
  #schema: T;
  constructor(schema: T) {
    super();
    this.#schema = schema;
...
class WArray<T> extends WType<T[]> {
  #inner: WType<T>;
...
class WString extends WType<string> {
  ...
  max(length: number) {
    this.validators.push((ctx) => {
      ...
      ctx.issues.push({
        path: [],
        message: `String is too short, maximun length: ${length}`,
      });
