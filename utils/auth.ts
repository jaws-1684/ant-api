...
import type { AuthenticatedRequest, UserDocument, UserDTO } from "../types/index.ts"
...
export const authorizedUser = async (
    { id, password }: 
    { id: string, password: string}): Promise<UserDocument> => {
...
export const matchPassword = async (plainTextPassword: string, hashedPassword: string) => {
    const match = await bcrypt.compare(plainTextPassword, hashedPassword);
    if (!match) return { ok: false, error: new InvalidCredentialsError() };
    return { ok: true }
}
export const isMatch = (match: { ok: true | false, error?: InvalidCredentialsError}) => {
