...
export class AppError extends Error {
  ...
  toObject = (): {
    status: number;
    error: { message: string; issues?: EIssues };
  } => ({
    status: this.status(),
...
export class ExternalError extends AppError {
  ...
  static errorFor = (e: Error): ExternalErrorConstructor =>
    ExternalError.externalErrorRegistry.find((cls) => cls.handles(e))!;
}
...
export class MongoNotFoundError extends ExternalError {
  ...
  status = () => 404;
  static handles = (e: Error) =>
    e instanceof MongooseError.DocumentNotFoundError;
  static {
    this.register(this);
  }
}
...
export const toAppError = (error: Error): AppError | null => {
  if (error instanceof WError)
    return new ValidationError(error.issues, "Validation error");
  if (error instanceof AppError) return error;
  if (ExternalError.canHandle(error))
    return new (ExternalError.errorFor(error))(error);
  return null;
};
