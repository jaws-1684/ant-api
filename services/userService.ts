...
import type { NewUser, UserDocument, UpdateUser, UserEntry } from '../types/index.ts';
import { hashPassword } from '../utils/auth.ts';


const addUser = async (newUser: NewUser, authProvider = 'local'): Promise<UserDocument> => {
    const { username, email, password } = newUser;
    const hash = await hashPassword(password);
    const user = await new User({ username, email, password: hash, authProvider }).save()
    return user;
}
const updateUser = async (updateUser: UpdateUser): Promise<UserDocument | null> => {
...
const updateRefreshToken = async ({ id, refreshToken }: { id: string, refreshToken: string })  => {
